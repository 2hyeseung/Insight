...
export default class Exercise_guide extends React.Component<ScreenProps,ScreenState> {
  ...
  async init(
    ...
    while(this.state.count<3){
      await this.isPose(this.state.pose);
      await this.delay(6000);
    }

    await console.log('가이드 완료')
    ...
    return(this.props.navigation.navigate(`${params.exerciseName}`))
  }
  ...
  async isObject(predictions){
    ...
    if(DetectedArr.length!=0){
      console.log('주변에 '+DetectedArr+'가 있어요. 조심하세요!');
      //(audio)주변에 방해물이 있는지 유의하세요!
  ...
  async isPose(pose){
      ...
      if ( keypoints.length == 17 ){
        ...
        //console.log('아직'+NotKeypointsArr+'가 카메라에 인식되지 않고 있어요!');
        console.log(this.state.count);
        console.log('-------------------------------------------');
